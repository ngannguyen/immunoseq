...
4/ Repeat steps (1,2,3) N times (N simulations). Calculate mean statistics
...
class Setup( Target ):
    ...
    def run(self):
        ...
        outdir = os.path.join( self.options.outdir, "sims" )
        ...
        if doneSims < numSim:
            ...
            summarydir = os.path.join(self.options.outdir, "summary")
            statsdir = os.path.join(self.options.outdir, "stats")
            system("mkdir -p %s" % statsdir)
            self.setFollowOnTarget( Summary(summarydir, self.options.samSize, self.options.numSamples, statsdir, numSim) )
...
class SimulationBatch( Target ):
    ...
    def run(self):
        for i in xrange(self.numSim):
            outdir = os.path.join(self.outdir, "sim-%d" %i)
            system("mkdir -p %s" %outdir)
            self.addChildTarget( Simulation(self.options, outdir) )
         
        #Combine the results
        sumdir = os.path.join(self.options.outdir, "summary", str(self.batchid))
        system("mkdir -p %s" %sumdir)
        self.setFollowOnTarget( Summary(self.outdir, self.options.samSize, self.options.numSamples, sumdir, 0) )


#------------- SIMULATION ---------------------
class Simulation( Target ):
    """Generate s simulations (corresponding to s samples/individuals)
    """
...
class Repertoire( Target ):
    ...
    def run(self):
        ...
        #Run different samplings:
        for samsize in self.samSizes:
            self.addChildTarget( Sampling(self.outdir, seqs, samsize) )
        
        #Run a replicate if it is sample 0 (sampling from the same repertoire)   
        sampleid = os.path.basename( self.outdir.rstrip('/') )
        if sampleid == '0':
            replicateDir = "%s_replicate" % self.outdir.rstrip('/')
            system("mkdir -p %s" %replicateDir)
            for samsize in self.samSizes:
                self.addChildTarget( Sampling(replicateDir, seqs, samsize) )

#++++++++++++++ SAMPLING from pseudoRepertoire ++++++++++++++++
class Sampling( Target ):
    """Sample from the input repertoire for "size" sequences
    """
    def __init__(self, outdir, seqs, samsize):
        Target.__init__(self)
        self.outdir = outdir
        self.seqs = seqs
        self.size = samsize
    ...
    def run(self):
        samseqs = sampling(self.seqs, self.size)

        #Pickling seqs:
        pickleFile = os.path.join(self.outdir, "%d.pickle" %self.size)
        pickle.dump( samseqs, gzip.open(pickleFile, "wb") )

#----------- OVERLAP -----------
class Overlap( Target ):
    def __init__(self, indir, outdir, numSamples, samSizes, cutoffs):
        Target.__init__(self, time=0.00025)
        self.indir = indir
        self.outdir = outdir
    ...
    def run(self):
        ...
        for i in xrange(self.numSamples -1 ):
            for j in xrange(i+1, self.numSamples):
                pairs.append( (str(i), str(j)) )
        
        #Calculate pairwise overlap:
        for pair in pairs:
            indir1 = os.path.join(self.indir, pair[0])
            indir2 = os.path.join(self.indir, pair[1])
            outdir = os.path.join(self.outdir, "%s-%s" %(pair[0], pair[1]))
            system("mkdir -p %s" %outdir)
            for samsize in self.samSizes:
                seqsFile1 = os.path.join(indir1, "%d.pickle" %samsize)
                seqsFile2 = os.path.join(indir2, "%d.pickle" %samsize)
                outfile = os.path.join(outdir, "%d.txt" %samsize)
                self.addChildTarget( OverlapPairwise(outfile, seqsFile1, seqsFile2, self.cutoffs) )
        #self.setFollowOnTarget( CleanupOverlap(self.indir) )

class OverlapPairwise( Target ):
    """Output file: %cutoff\tclones1\tclones2\toverlap\t%1overlap2\t%2overlap1\t%reads1overlap2\t%reads2overlap1
    """
    def __init__(self, outfile, seqsFile1, seqsFile2, cutoffs):
        Target.__init__(self)
        self.outfile = outfile
        self.seqsFile1 = seqsFile1
        self.seqsFile2 = seqsFile2
        self.cutoffs = cutoffs
...
class Summary( Target ):
    def __init__(self, indir, samSizes, numSamples, outdir, denominator):
        Target.__init__(self)
        self.indir = indir
        self.samSizes = samSizes
        self.numSamples = numSamples
    ...
    def run(self):
        ...
        for p in pairs:
            pair = "%s-%s" %( p[0], p[1] )
            outdir = os.path.join(self.outdir, pair)
            system("mkdir -p %s" %outdir)
            for samsize in self.samSizes:
                self.addChildTarget( SummarySamsize(self.indir, samsize, pair, outdir, self.denom) )
        self.setFollowOnTarget( CleanupSummary(self.indir) )

class SummarySamsize( Target ):
    def __init__(self, indir, samsize, pair, outdir, denominator):
        Target.__init__(self)
        self.indir = indir
...
def addOptions( parser ):
    ...
    parser.add_option("-N", "--numSim", dest="numSim", type="int", default=100, help="Number of simulations. Default = 100")
    ...
    parser.add_option("-S", "--samplingSize", dest="samSize", default="50000,100000,150000,200000,500000", help="Comma separated string of sampling sizes. Default='50000,100000,150000,200000,500000'")
    ...
    parser.add_option("-p", "--cutoffs", dest="cutoffs", default="0,0.001,0.01,0.05,0.1,0.5,1,5,10", help="Comma separated clonesize percentage cutoffs. Default='0,0.001,0.01,0.05,0.1,0.5,1,5,10'")
